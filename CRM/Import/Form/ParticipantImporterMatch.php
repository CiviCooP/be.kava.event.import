...
class CRM_Import_Form_ParticipantImporterMatch extends CRM_Core_Form {
	...
	public $fields, $unknownParticipants = array(), $event_id, $status_id, $role_id, $json, $new = 0, $existing = 0, $matched = 0;
	...
	function buildQuickForm() {
		...
		if($_SERVER['REQUEST_METHOD'] == "GET") {
			...
			foreach($this->unknownParticipants as $participant) $this->addEntityRef("con_".$participant['id'], ts($participant['first_name_1']." ".$participant['last_name']));
			$this->assign('elementNames', $this->getRenderableElementNames());
			parent::buildQuickForm();	
		}
		
		/* Handle the form */
		if($_SERVER['REQUEST_METHOD'] == "POST") $this->postHandler();
	}
	...
	function postHandler() {
		foreach($_POST as $id => $match) {
			if(!stristr($id, "con_")) continue;
			$id = substr($id, 4);
			if(empty($match)) $participantIdentifier = $this->createContact($this->unknownParticipants[$id]);
			if(!empty($match)) {
